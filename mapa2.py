...
class Mapa():
    def __init__(self):
        ...
        self.posicion_objetivo_anterior = None
        ...
        if detector.obtener_resultados:
            ...
            if pos_jugador is not None:
                ...
                if nodos_raw is not None:
                    ...
                    if self.ruta:
                        if self.posicion_objetivo_anterior is not None:
                            desplazamiento = (
                                self.ruta_filtrada[-1][0] - self.posicion_objetivo_anterior[0],
                                self.ruta_filtrada[-1][1] - self.posicion_objetivo_anterior[1]
                            )
                            self.ajustar_nodos_por_desplazamiento(desplazamiento)
                        self.posicion_objetivo_anterior = (self.ruta_filtrada[-1][0], self.ruta_filtrada[-1][1])
                    else:
    ...
    def ajustar_nodos_por_desplazamiento(self, desplazamiento):
        for i in range(len(self.ruta)):
            self.ruta[i] = (self.ruta[i][0] + desplazamiento[0], self.ruta[i][1] + desplazamiento[1])
        self._filter_nodes_by_distance(step=100)
        self._draw_nodes_on_map()
    ...
    def mask_to_navigation_matrix(self):
        ...
        self.matrix[np.all(self.mapa_color == [255, 255, 255], axis=-1)] = 0  
        self.matrix[np.all(self.mapa_color == [0, 0, 0], axis=-1)] = 1  
        self.matrix[np.all(self.mapa_color == [0, 0, 255], axis=-1)] = 2  
        self.matrix[np.all(self.mapa_color == [255, 0, 0], axis=-1)] = 3  
    ...
    def astar(self, start, goal):
        ...
        while open_list:
            ...
            for neighbor in self.get_neighbors(current):
                if self.matrix[neighbor[1], neighbor[0]] == 1:
                    continue
